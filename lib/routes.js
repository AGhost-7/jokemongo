...
async function getJoke(ctx, next) {
  const joke = await ctx.db.randomJoke()
  if (!joke) {
...
async function postJoke(ctx) {
  await ctx.db.submitJoke(ctx.request.body)
  ctx.status = 204
  ctx.body = ''
}

async function upvote(ctx, counter) {
  const vote = await ctx.db.submitVote(counter, true)
  ctx.status = vote ? 204 : 404
  ctx.body = '👍\n'
}

async function downvote(ctx, counter) {
  const vote = await ctx.db.submitVote(counter, false)
  ctx.status = vote ? 204 : 404
  ctx.body = '👎\n'
}
